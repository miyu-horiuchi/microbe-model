...
**On the magnitude of improvement.** Stated as percent reductions in error against the prior-work predictors whose splits and baselines are matched closely enough to support a direct comparison, the gains are substantial. On the same 5,000-genome family-heldout manifest, this work reduces optimum temperature MAE by 39% (4.39 → 2.67 °C), pH MAE by 23% (0.61 → 0.47), and salt MAE by 3% (1.98 → 1.92%) relative to GenomeSPOT. Against the strongest published BacDive baseline (Koblitz et al. 2025, Pfam-only random forest on a 21K-strain corpus), this work reduces temperature MAE by approximately 9% (2.94 → 2.67 °C) on roughly twice the corpus. For oxygen classification on fold 0, LoRA fine tuning of ESM 2 t12 on HMM-gated marker sequences raises four-class macro-F1 by 135% relative to the tabular head (0.402 → 0.945). For medium recommendation on a 5-fold family-heldout dry-lab split, the XGBoost recommender improves Hit@5 by 108% relative to the strongest popularity baseline (0.372 → 0.775). Comparisons to Li 2023 and Máša 2025 differ in task and cannot be reduced to a single percent number; they are listed in Table 5 for context.
